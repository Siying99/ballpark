...
### 13.1 Aggregate block: encoded in a follow-up PR

> **Status update (follow-up PR, June 2026).** The aggregate block has now been **encoded** in `aggregate-draft.yaml`, with the household↔aggregate wiring in `openha-ge-draft.yaml`, exactly along the blueprint recorded below. This subsection is retained as the canonical record of (a) why the household-only scope was the right choice for the *original* PR #73, and (b) the blueprint that the follow-up PR implemented. The block-level dolo-plus-syntax workaround introduced by the aggregate YAML is recorded in the new §13.5. The scope fields in `OpenHA.md` frontmatter and the callouts in `AGENTS.md` have been updated from `household-block-only` to `household-and-aggregate` accordingly.

**Original status (PR #73).** The household three-stage period of §8 is encoded in `dolo-plus-draft.yaml`. The aggregate / GE closure of §§3–4 — equations E.1–E.16 in the consolidated system of §7 — is **not** encoded.
...
**What the follow-up PR should contain** (and what was delivered).

- ✅ A new `aggregate-draft.yaml` in this directory implementing the 16 equations consolidated in §7 above. The natural composition is (a) one block grouping E.3–E.7 (price/exchange-rate identities), (b) one block grouping E.8–E.10 + E.13 (firm/dividend/production), (c) one block for E.9 (wage Phillips curve), (d) one block for E.11–E.12 (monetary rules), (e) one block for E.2 (goods-market clearing), (f) one block for E.14–E.16 (NFA/current-account/terminal conditions). **Delivered:** `aggregate-draft.yaml` encodes exactly these six blocks (`prices_fx`, `firms`, `wage_phillips`, `monetary`, `goods_market`, `nfa_ca`), with every equation tagged `E.1`–`E.16` to match §7, plus a `unknowns`/`targets` partition for the sequence-space GE solve and a paper-Table-2 calibration block.
- ✅ Wiring of `dolo-plus-draft.yaml` (household) and `aggregate-draft.yaml` (aggregate) into a higher-level GE composition file. The household-block `r`, `w`, `N` parameters become inputs from the aggregate block; the household-block aggregate consumption $C_t$ becomes an input to the aggregate-block goods-market-clearing equation E.2. **Delivered:** `openha-ge-draft.yaml` declares both blocks as `components`, the bidirectional `wiring` (`(r,w,N) → household → (C,A) → aggregate → (r,w,N)`), the equilibrium selection rule, the MIT-shock drivers, and the steady state. Per §13.4, the household YAML's `w`, `N`, `r` parameters are documented there as becoming time-varying inputs once wired.
- ⏳ A renewed Matsya pass (continue session `topics2026-siying99-ballpark`) on the full `bellman-excerpt.md` to flag any DDSL issues with §§3, 5, 6, 7 that did not surface in the household-block-only round 8 iteration. **Remaining human-in-the-loop step:** the aggregate YAML in this PR is the Cursor/Claude *Extract* draft (CONTRIBUTING.md "formalization iteration", step 1). The Matsya `evaluate` pass on the aggregate block (steps 2–4) is the next action and is recorded as such in `verification.md` (Round 10) and `AGENTS.md`.
...
### 13.5 Aggregate-block container syntax (SPECULATIVE; introduced by the follow-up PR)

**Status.** The follow-up PR (§13.1 status update) added `aggregate-draft.yaml` and `openha-ge-draft.yaml`. dolo-plus has **no canonical syntax** for (i) an aggregate / general-equilibrium *sequence-space* block — a system of equilibrium conditions solved jointly for sequences, rather than a household perch decomposition — or (ii) the composition of a heterogeneous-agent household block with such an aggregate block into a GE fixed point. The container layout used (`blocks` with per-block `inputs`/`outputs`/`equations`; top-level `unknowns`/`targets`; `consumption_functional` provider reference; and, in the GE file, `components`/`wiring`/`equilibrium`/`shocks`/`steady_state`) is therefore **SPECULATIVE at the keyword level**.

**Why this is a workaround, not a fudge.** This is the same class of spec gap already recorded in this repository for the Benhabib–Bisin–Luo (2019) cross-generational layer (`dolo-plus-dynasty.yaml`), where Matsya's 2026-04-27 review confirmed UNRESOLVED status at the dolo-plus spec level for aggregate-style / composition-style constructs (calibration-override families, terminal-boundary blocks, period templates with state-dependent parameters). The **economics is final and paper-grounded** — the 16 equations are transcribed verbatim from §7, every one tagged `E.1`–`E.16`; only the *container keywords* are provisional and may be renamed once a canonical sequence-space-Jacobian (SSJ) idiom appears in the dolo-plus spec.

**Specific sub-items flagged inline** (per CONTRIBUTING.md line 47, at the point of divergence):

- **Container kinds** `kind: sequence-space-block` (aggregate) and `kind: ge-fixed-point` (composition) are not canonical dolo-plus `kind` values (block-level `# unresolved (SPECULATIVE block-level):` flag at the head of each new YAML). *These names were adopted in the Matsya round-11 evaluate pass (see below), replacing the original `aggregate-block-system` / `ge-composition`, on Matsya's suggestion that they align better with the SSJ literature ("sequence-space block") and emphasise the fixed-point (not pipeline) nature of the composition.*
- **Aggregation of household policies (`C, A`).** The household YAML defines *individual* policies `c(m,e)`, `psi(m,e)`; producing the *aggregates* `C = ∫ c dD`, `A = ∫ a^p dD` over the cross-sectional distribution requires a distribution → aggregation → Jacobian pipeline that has **no canonical dolo-plus syntax**. The `consumption_functional` `produces: [C, A]` declaration is the contract; the aggregation step itself is implicit (an SSJ toolkit / HARK instantiation supplies it). Flagged at the `consumption_functional` block. *(Identified in the Matsya round-11 evaluate pass, Issue 2.4.)*
- **Terminal / boundary conditions (E.16).** Horizon-end conditions `nfa_T = 0`, `Q_T = Q_ss`, `p_T = d_ss/r_ss` imposed at the truncation horizon `T` (declared in `settings: T: 300`) have no canonical dolo-plus boundary idiom; encoded under a `terminal_conditions` key (now also separated from per-period `targets` into a `boundary_conditions` list) and flagged in the `nfa_ca` block.
- **`unknowns`/`targets` partition.** The split into solver unknowns and zeroed residuals is an SSJ modelling choice; the standard small-open-economy HANK partition is used and flagged at the `unknowns` declaration. *(Refined in the Matsya round-11 evaluate pass: `pi_w` dropped from the unknowns since it is a deterministic helper of `W`; the partition is now a well-posed 4×T square system with E.16 as boundary, not target.)*
- **Derived parameter `kappa_w`.** Whether the wage-PC slope should be a free calibration entry or computed from `(beta, theta_w)` inside the block is convention-dependent; listed explicitly with the arithmetic shown, and flagged. *(Its value was corrected from `0.00408` to the formula value `0.006268` in the Matsya round-11 evaluate pass, Issue 1.2.)*

**Resolved (Matsya round-11 evaluate pass).** The **real-UIP placement (E.7)** question — within-period vs between-period connector, flagged as open in §13.1 — is now **resolved**: in the sequence-space framework all forward-looking conditions (E.7, E.9, E.11, E.15) are per-period residuals over sequences, so the within-system encoding over `{Q_t}` is correct. The between-period-connector reading would apply only if `Q` were a household-level perch state, which it is not (it is an aggregate price). The inline `prices_fx` comment has been upgraded from "open question" to "resolved."

**Inline flags.** Both new YAMLs carry a block-level `# unresolved (SPECULATIVE block-level): ... see bellman-excerpt.md §13.5` comment in their preambles, and per-item `# unresolved:` / `# workaround:` / `# resolved:` comments at each of the points above. This §13.5 is the **canonical** cross-document record.
