...
- **Container kinds** `kind: aggregate-block-system` and `kind: ge-composition` are not canonical dolo-plus `kind` values (block-level `# unresolved (SPECULATIVE block-level):` flag at the head of each new YAML).
- **Real-UIP placement (E.7).** Whether the real-UIP arbitrage `1+r = (1+r*) Q[+1]/Q` is a within-period equation or a between-period (twister-like) connector is a modelling choice (this was anticipated in §13.1's "non-trivial dolo-plus modelling choices"). Encoded as a within-system equation over `{Q_t}` with a forward lead `Q[+1]`; flagged inline in the `prices_fx` block.
- **Terminal / boundary conditions (E.16).** Horizon-end conditions `nfa_∞ = 0`, `Q_∞ = Q_ss` in a stationary infinite-horizon MIT-shock problem have no canonical dolo-plus boundary idiom; encoded under a `terminal_conditions` key and flagged in the `nfa_ca` block.
- **`unknowns`/`targets` partition.** The split into solver unknowns and zeroed residuals is an SSJ modelling choice; the standard small-open-economy HANK partition is used and flagged at the `unknowns` declaration.
- **Derived parameter `kappa_w`.** Whether the wage-PC slope should be a free calibration entry or computed from `(beta, theta_w)` inside the block is convention-dependent; listed explicitly with the arithmetic shown, and flagged.

**Inline flags.** Both new YAMLs carry a block-level `# unresolved (SPECULATIVE block-level): ... see bellman-excerpt.md §13.5` comment in their preambles, and per-item `# unresolved:` / `# workaround:` comments at each of the points above. This §13.5 is the **canonical** cross-document record.
